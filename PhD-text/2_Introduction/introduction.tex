...
For example, the binding energy\footnote{More precisely it is a ground state energy, nut I follow commonly used \tmp{mental} shortcut.}
...
However, electromagnetic interaction between nuclei manifests only at very 
...
At higher energies agreement is observed but is getting worse.
...
However, in the thesis, I stick with electromagnetic processes.
...
in 2001 \cite{epelbaum_physrevc_2002} \gls{n2lo} model which included 3NF contributions \tmp{cut page???}.
\tmp{\gls{n3lo} potential in 2005 \cite{epelbaum2005two} which included 3NF contributions}.
...
An important step was done when authors started using a semi-local regularisation 
...
\cite{reinkrebs2018}: the \gls{sms}. It is developed up to \gls{n4lo+} at the moment.
...
investigations \cite{Machleidt2005, Machleidt2010, Entem2017}\tmp{what to add?} introduced very similar model
...
of nuclei properties \tmp{and nuclear matter} without using 3NF.
...
Let me add, that another regularisation function used by R.~Machleidt and collaborators 
with non-local regulator only \cite{Entem2003, Entem2017}.
...
While the derivation of such currents is ongoing \tmp{[H.~Krebs, private communication]}, at the moment
