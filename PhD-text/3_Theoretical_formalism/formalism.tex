...
\section{Deuteron bound state}
    ...
    \begin{figure}[h]
        ...
        as a mean value over all cutoffs.
    ...
    \begin{figure}[h]
        ...
        \end{center}
        \caption{Wave function $\phi_l$ for l=0 ($3S_1$ partial wave) - left figure and l=2 ($3D_1$) - right figure.}
        \label{wave_func}
    \end{figure}
...
\section{Theoretical uncertainties}
   ...
   may affect a quality of the prediction.
    ...
    The \gls{sms} model at N$^4$LO+ is used.
    ...
    At the moment we have only a single nucleon 
    current consistent with the model available. In most cases it is not enough 
    to proper describe regarded processes which will be confirmed in Chapter~\ref{chap:results}.
    We deal with this problem applying the Siegert theorem which allows to include
    some contribution from the 2N current, which is still not complete.
