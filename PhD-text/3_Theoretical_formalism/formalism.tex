...
\label{nd_state}
    ...
    \begin{equation}
        \begin{aligned}
        \left\langle\vec{p}^{\prime}\left| J_1^\mu(0)\right| \vec{p}\right\rangle & =\bar{u}\left(\vec{p}^{\prime} s^{\prime}\right)\left(\gamma^\mu F_1+i \sigma^{\mu \nu}\left(p^{\prime}-p\right)_\nu F_2\right) u(\vec{p} s) \\
    ...
    ie. including two- and three- nucleon interaction,, the symbols $u$ represent Dirac spinors, $F_1\left(\left(p^{\prime}-p\right)^2\right)$ and $F_2\left(\left(p^{\prime}-p\right)^2\right)$ correspond to the Dirac and Pauli form factors of the nucleon, respectively, and $G_M \equiv F_1+2 M_N F_2$ denotes the magnetic form factor of the nucleon. The proton charge $e$ is extracted from the matrix element. In this thesis, only the nonrelativistic limit of \eq{current_general} is considered, which leads to well-known expressions for the operators for convection and spin currents:


    \begin{equation}
        \begin{array}{r}
            \left\langle\vec{p}^{\prime}\left| J_1^0(0)\right| \vec{p}\right\rangle=\left(G_E^p \Pi^p+G_E^n \Pi^n\right), \\
            \left\langle\vec{p}^{\prime}\left| \vec{J}_1(0)\right| \vec{p}\right\rangle=\frac{\vec{p}+\vec{p}^{\prime}}{2 M_N}\left(G_E^p \Pi^p+G_E^n \Pi^n\right)+\frac{i}{2 M_N}\left(G_M^p \Pi^p+G_M^n \Pi^n\right) \vec{\sigma} \times\left(\vec{p}^{\prime}-\vec{p}\right) .
        \end{array}
    \end{equation}

    The electric form factor, denoted as $G_E$, is defined as,
    ie. including two- and three- nucleon interaction, $G_E \equiv F_1+\frac{\left(p^{\prime}-p\right)^2}{2 M_N} F_2$. It represents the neutron (n) and proton (p) form factors. These form factors, $G_E$ and $G_M$, are normalized according to the following,
    ie. including two- and three- nucleon interaction, conditions:
    ...
    The two-nucleon current contribution is partially taken into account via Siegert approach \cite{Siegert, GolakKamad2000_ExplDescr, Golak2005}.
...
\section{Theoretical uncertainties}
   ...
   may affect a quality of the prediction.
    ...
    in order to obtain the potential parameters \cite{reinkrebs2018}. As any fitting, it 
