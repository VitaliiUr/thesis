...
\section{2N bound state}
    ...
    The time-independent Schr\"{o}din\-ger equation for two particles is expressed as:
    ...
    wave function.
    In PWD it results in the additional requirement for allowed quantum numbers which is:
...
\section{2N scattering state}
    ...
    I solve \eq{lse_pwd_reduced} numerically, which again requires discretization
...
\label{nd_state}
    ...
    Each $j_\mu^n$ describes the interaction of a photon with all relevant permutations of $n$ nucleons,
    i.e. $j_\mu^1$ is a sum of photon interactions with each of the nucleons in the system separately. 
    ...
    This is acceptable because, in a low-energy regime, contributions from many nucleons to the nuclear charge density are typically small.
    ...
    In \eq{3n_conv} and \eq{3n_spin} we present convection and spin currents which combined 
   ...
   \begin{eqnarray}
    ...
    The natural domain is defined by the energies of outgoing nucleons    
