...
\label{sec:hel_3N}
    ...
    Figs.\ref{CROSS_HE_EXCL_165_15_15_165}.
        ...
        \begin{figure}[h]
            ...
            % \tmp{stylistic chages: axis label shift to left, change units to pb}}
...
\section{Triton photodisintegration}
    ...
    \begin{figure}[h]
        \begin{center}
            ...
            energy E$_\gamma=\SI{30}{\mev}$ for the kinematic configuration
            $\theta_1 = \ang{15}$, $\phi_1 = \ang{0}$,
            $\theta_2 = \ang{15}$, $\phi_2 = \ang{180}$ with two neuterons detected \tmp{check ???}.
...
\begin{figure}[h]
        ...
        \caption{
            ...
            ground-state broadening in pionic deuterium and taken from Ref.~\cite{Strauch2010,Strauch2011}.}
        \label{Gamma_nn}
    \end{figure}

    In \fig{Gamma_nn} the absorption rate $\Gamma_{nn}$ is shown for the process $\pi^- + {^2{\rm H}} \rightarrow n + n$
    as a dependance on the chiral order (horizontal axis) and cutoff parameter (different lines) as well
    as experimental data extracted from \cite{Strauch2010,Strauch2011}. We see the noticeable difference between
    predictions obtained using different $\Lambda$. The impact of chiral orders is large at \gls{lo}
    and \gls{nlo} and is gradually decreasing, and \gls{n3lo}-\gls{n4lo}-\gls{n4lo+} steps are almost invisible.
    The comparison with experimental data shows that our predications converge to a correct value coming 
    closely to it al large chiral orders.
    ...
    At the next order, \gls{n4lo}, it comes back to the \tmp{normal} arrangement.
    This behavior may be connected to the $^3$He wave function resultingfrom the
    3NF used for the calculation.
    In order to check that I show in \fig{proton_rad}
    ...
    similar deviation at \gls{n3lo} while radious obtained without the 3NF does not.
    ...
    \begin{figure}[h]
        ...
        Predictions were obtained with the \gls{sms} NN interactionat given order combined
        with \gls{n2lo} 3NF.}
        \label{Gamma_pnn}
    \end{figure}
    ...
    Let us remind that in case of absorption rates only two kinetical variables are
    neccessary to define exclusive kinematical configuration. \tmp{That is, because for that
    process there is full rotational symmetry, as none of vectors define the S-axis.}
    ...
    process as functions of the nucleons energies (assumingnucleon number 1 to be proton) and 
