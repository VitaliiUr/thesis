...
\label{sec:de_results}
    ...
    \end{equation}
    so in the specific case of comparison the data with predictions based on \gls{snc} current and "\gls{snc} + Siegert" we
    ...
    % To some extent
    % the Siegert approach works  as no additional 2N contributions are taken into account.
    However, \fig{TOTAL_CROSS_small} reveals the approximated character of the Siegert approach.
    ...
    (see e.g. \fig{Diff_cross_cutoff} below) or by the low order of chiral potential (see e.g. \fig{Trunc_100} or \fig{Diff_cross_truncation} below).
    ...
    around \SI{4.5}{\percent} for \SI{100}{\mev} and \SI{8}{\percent} for \SI{140}{\mev}
     ...
     It means that even cut-off dependence for that total cross section remains
     relatively small, especially at the lower energies.
     Note, that the biggest relative difference arise from regions 
    ...
    All the analyzing powers present here show excellent convergence 
    ...
    In \fig{tensor_angular_230-330} the description of data points for T$_{20}$ seems to be better
    ...
    The \fig{T20_vs_en_cutoff} presents analogous results to those given in \fig{T20_vs_en} but
    ...
    Similar picture is seen in the Figs \ref{tensor_energy_24-48} and \ref{tensor_energy_70-102}
    ...
    The description is better for bigger $\theta_p$ (\fig{tensor_energy_70-102}):
...
\label{sec:hel_3N}
    ...
    Similar trends are present also in other configurations, demonstrated for the comparison in
    \fig{CROSS_HE_EXCL_15_105_15_75},
    \fig{CROSS_HE_EXCL_45_75_45_105} and
    \fig{CROSS_HE_EXCL_165_15_15_165}.
        ...
        \begin{figure}[h]
            \begin{center}
                ...
                \end{center}
                \caption{The same as in the \fig{CROSS_HE_EXCL_15_105_15_75} but for the kinematic
                configuration at
...
\subsection{Two-body breakup}
    ...
    is shown in the \fig{CROSS_nd_30} (for the photon energy $\text{E}_\gamma = \SI{30}{\mev}$)
...
\begin{figure}[h]
    \begin{center}
        ...
        The initial photon energy $\text{E}_\gamma=\SI{30}{\mev}$, NN with 2NF and \gls{snc}+Siegert model are used.}
        \label{CROSS_nd_30}
    \end{figure}
...
\section{Triton 3N photodisintegration}
    ...
    In the \fig{CROSS_Triton_EXCL_30_15_0_15_180} I show the a differential cross section 
    $\frac{d^5\sigma}{d\Omega_1d\Omega_2dS}$ as a function of the S-arclength.
    As for the example present in \fig{CROSS_HE_EXCL_30} I use
...
In the following subsections, I present my results for the pion absorption processes
...
Thus in the following, I show predictions of the differential and total capture rates
...
\begin{figure}[h]
    ...
    In \fig{Gamma_pnn} and \ref{Gamma_nd} the total pion absorption rates are present as a function
    ...
    This is a situation of quasi-free scattering(QFS) - when a proton is a spectator and both neutrons share equally all energy. In \fig{pion_map_xy_cutoff} this region is located at $y \rightarrow -1$ and $x \rightarrow 0$. 
    ...
    is present in  \fig{pion_map_xy_cutoff} at $y \rightarrow 1$.
    ...
    \begin{figure}[h]
        ...
        All other contributions are the same as in \fig{pion_map_E1E2_cutoff}: \gls{pw}+rescattering and 2NF+3NF.
        Each panel presents predictions obtained with different values of the cut-off parameter $\Lambda$:
        from $\Lambda = \SI{400}{\mev}$ (upper left) to $\Lambda = \SI{550}{\mev}$ (lower right). Nucleon 1 is a proton.}
    ...
    Figs.~\ref{pion_map_E1E2_order} and \ref{pion_map_xy_order} show results obtained using similar
    ...
    that is the same as in e.g. \fig{pion_map_E1E2_order}, but integrated over E$_n$($\equiv\text{E}_2$).
    ...
    The right panel of the \fig{pion_GdEp} shows a cut-off dependence of the $d\Gamma_{pnn} /d\text{E}_p$ obtained
    ...
    \fig{pion_map_E1E2_cutoff}, but from the current perspective it is clearly present.
    ...
    Similarly in \fig{pion_dGdEn} $d\Gamma_{pnn} /d\text{E}_n$ is present. We observe similar trends as above.
    ...
    Moving on to the next figures, \fig{pion_dGdEr} and \fig{pion_dGdphi}, I show 1-dim dependence of the
    ...
    both single- and two-nucleon currents and two-nucleon plus three-nucleon (at \gls{n2lo}) interaction.
    ...
    My predictions from the \fig{Gamma_nnn} can be summarized by the value from \gls{n4lo+}:
    ...
    It is because of the symmetry of the three-body system as there are three identical particles - neutrons in the final state.
    ...
    Same regions are placed in \fig{pion_nnn_xy_cutoff} around $(x, y) = (0, -1), (-0.85, 0.5)$ and $(0.85, 0.5)$, respectively.
    ...
    the \fig{pion_nnn_E1E2_cutoff_PW}
    and $(x, y) = (0, 1), (0.75, -0.5)$ and $(-0.75, -0.5)$ in the \fig{pion_nnn_xy_cutoff_PW}
    ...
    different structure and even scale. The contribution from 2N current is not 
    ...
    This once more confirms the importance of the inclusion of a proper 2N current in the model. 

    Figs.~\ref{pion_nnn_E1E2_order} and \ref{pion_nnn_xy_order} show similar intensity plots to those shown in
    ...
    shows very similar distributions of the absorption rate,
    except for the \gls{n2lo} one, where the central region (with lowest $\Gamma_{nnn}$ values) is less pronounced.

    It is further confirmed in the \fig{pion_dGdEn_3H} (left panel) where differential absorption rate $d\Gamma_{nnn} /d\text{E}_n$ at \gls{n2lo} has much larger peaks (at around $E_n = \SI{0}{\mev}$ and $E_n = \SI{68}{\mev}$)
    ...
    On the right panel of \fig{pion_dGdEn_3H} we see that only the prediction obtained with $\Lambda=\SI{400}{\mev}$ differs from others. It is interesting to notice, that such a difference is nearly invisible from the intensity plots.

    A very similar tendency is observed in Figs.~\ref{pion_dGdr_3H} and \ref{pion_dGdphi_3H},
    where I show the differential absorption rate with respect to the Dalitz radial coordinates $r$ and $\phi$, respectively. As in \fig{pion_dGdEn_3H} we see that \gls{n2lo} and $\Lambda = \SI{400}{\mev} $ predictions are different from other results, while all the other curves are very close to each other.
    ...
    \begin{figure}[h]
        ...
        $d^2 \Gamma_{nnn}/dE_1dE_2$ for the $\pi^- + ^3\text{H} \rightarrow n + n + n$
        process obtained using the SMS NN potential at \gls{n4lo+} supplemented by \gls{n2lo} 3NF
        with all ingredients possible: \gls{pw} only (without rescattering), 1NC+2NC, 2NF+3NF.
        Each panel presents predictions obtained with different values of the cut-off parameter $\Lambda$:
        from \SI{400}{\mev} (upper left) to \SI{550}{\mev} (lower right).}
    ...
    \begin{figure}[h]
        ...
        \end{center}
        \caption{The same as in \fig{pion_nnn_E1E2_cutoff_PW} but for the double differential absorption rates
        $d^2 \Gamma_{nnn}/dxdy$.}
    ...
    \begin{figure}[h]
        ...
        $d^2 \Gamma_{nnn}/dE_1dE_2$ for the $\pi^- + ^3\text{H} \rightarrow n + n + n$
        process obtained using the SMS NN potential at \gls{n4lo+} supplemented by \gls{n2lo} 3NF
        with all ingredients possible: \gls{pw} + rescattering, 1NC, 2NF+3NF.
        Each panel presents predictions obtained with different values of the cut-off parameter $\Lambda$:
        from \SI{400}{\mev} (upper left) to \SI{550}{\mev} (lower right).}
    ...
    \begin{figure}[h]
        ...
        \end{center}
        \caption{The same as in \fig{pion_nnn_E1E2_cutoff_1NC} but for the double differential absorption rates
        $d^2 \Gamma_{nnn}/dxdy$.}
